...
def merge_with_input(input_file, new_data):
    try:
        ...
        sys.exit(1)
        # existing_data = {} //incase if need to send only metadata
    ...
    infra_metadata = existing_data.get("Infra_Metadata", {})
    infra_metadata.update(new_data)
    existing_data["Infra_Metadata"] = infra_metadata
...
def trigger_github_action(repository, merged_file):
    ...
    if response.status_code == 204:
        ...
        print(f"Failed to trigger GitHub Repo '{repository}'. Status code: {response.status_code}")
        print(response.text)
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    file_prefix =  "cam" # os.getenv('jsontemp_Platform')
    region = "eastus2" #os.getenv('jsontemp_Region')
    environment = "dev" #os.getenv('jsontemp_Sub_Environment')
    mapping_path = "mapping" #os.getenv('mapping_path', '')
    temp_json_path = "mapping" #os.getenv('temp_json_path')
    ...
    if result is not None:
        input_file = os.path.join(temp_json_path, 'input-temp.json')
